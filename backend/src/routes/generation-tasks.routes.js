...
router.get('/', async (req, res) => res.json(await svc.listTasks({
...
router.delete('/:taskId', async (req, res) => { const task = await svc.deleteTask(req.params.projectId, req.params.taskId, req.body || {}); if (!task) return res.status(404).json({ message: 'Task not found.' }); res.json(task); });
router.post('/:taskId/restore', async (req, res) => { const task = await svc.restoreTask(req.params.projectId, req.params.taskId); if (!task) return res.status(404).json({ message: 'Task not found.' }); res.json(task); });
router.delete('/:taskId/permanent', async (req, res) => { const result = await svc.permanentlyDeleteTask(req.params.projectId, req.params.taskId); if (!result) return res.status(404).json({ message: 'Task not found.' }); res.json(result); });
